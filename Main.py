...
class BillingSystem(ctk.CTkToplevel):
    def __init__(self, master=None, user_role="admin"):
        ...
        self.title(f"Sistema de Facturación AS — v2.7 ({'Administrador' if user_role == 'admin' else 'Empleado'})")
    ...
    def open_settings_dialog(self):
        ...
        # Botones para consultar tasas
        query_frame = ctk.CTkFrame(dialog, fg_color="transparent")
        query_frame.pack(fill="x", padx=20, pady=5)
        
        # Status Label para las consultas de tasas
        status_label = ctk.CTkLabel(dialog, text="Consulte la tasa del dólar en tiempo real", font=("Segoe UI", 11, "italic"), text_color="#64748B")
        
        def fetch_bcv():
            status_label.configure(text="Consultando tasa BCV...", text_color="#2563EB")
            bcv_btn.configure(state="disabled")
            para_btn.configure(state="disabled")
            ...
            def run():
                ...
                def update_gui():
                    ...
                    para_btn.configure(state="normal")
                    if rate is not None:
        ...
        def fetch_parallel():
            status_label.configure(text="Consultando tasa Paralelo...", text_color="#2563EB")
            bcv_btn.configure(state="disabled")
            para_btn.configure(state="disabled")
            
            import threading
            def run():
                rate, err = self.query_dolar_api("paralelo")
                ...
                def update_gui():
                    ...
                    if rate is not None:
                        exchange_entry.delete(0, "end")
                        exchange_entry.insert(0, f"{rate:.2f}")
                        status_label.configure(text=f"Tasa Paralelo obtenida: {rate:.2f} VES", text_color="#059669")
                    else:
                        status_label.configure(text=f"Error: {err}", text_color="#DC2626")
        ...
        bcv_btn = ctk.CTkButton(
            query_frame, 
            text="Consultar BCV 🏛️", 
            command=fetch_bcv, 
            width=190,
            fg_color="#3B82F6",
            hover_color="#2563EB"
        )
        bcv_btn.pack(side="left", padx=(10, 5), pady=5)
        
        para_btn = ctk.CTkButton(
            query_frame, 
            text="Consultar Paralelo 📈", 
            command=fetch_parallel, 
            width=190,
            fg_color="#3B82F6",
            hover_color="#2563EB"
        )
        para_btn.pack(side="right", padx=(5, 10), pady=5)
        ...
        auto_cb = ctk.CTkCheckBox(auto_frame, text="Auto-actualizar tasa", variable=auto_var, font=("Segoe UI", 12))
        auto_cb.pack(side="left", padx=10, pady=5)
        
        source_var = tk.StringVar(value="BCV" if getattr(self, "rate_source", "oficial") == "oficial" else "Paralelo")
        source_combo = ttk.Combobox(auto_frame, textvariable=source_var, values=["BCV", "Paralelo"], state="readonly", width=12)
        source_combo.pack(side="right", padx=10, pady=5)
        ...
        ctk.CTkLabel(folder_frame, text="Exportaciones Excel:", font=("Segoe UI", 12)).pack(side="left", padx=(0, 8))
        ctk.CTkButton(
            folder_frame,
            text="Abrir carpeta Excel",
        ...
        def save_settings():
            try:
                ...
                self.rate_source = "oficial" if source_combo.get() == "BCV" else "paralelo"
                self.save_data()
...
"""
    ...
    def export_invoices_excel(self):
        ...
        if not self.invoices:
            messagebox.showwarning("Sin Facturas", "No hay facturas para exportar.")
            return
        ...
        if exported_files:
            ...
            messagebox.showwarning("Exportación vacía", "No se exportaron facturas.")
    ...
    def open_excel_export_folder(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudo abrir la carpeta Excel:\n{str(e)}")
    ...
    def open_edit_user_from_tab(self):
        """Editar o recuperar contraseña de un usuario sin requerir contraseña anterior"""
        selection = self.users_tree.selection()
        if not selection:
            messagebox.showwarning("Sin selección", "Seleccione un usuario para editar", parent=self)
            return
    ...
    def delete_user_from_tab(self):
        """Eliminar un usuario desde la pestaña de administración"""
        selection = self.users_tree.selection()
        if not selection:
        ...
        confirm = simpledialog.askstring(
            "Confirmar Eliminación",
            f"Para eliminar permanentemente al usuario '{username}',\nescribe la palabra: borrar",
            parent=self
        )
        if confirm and confirm.strip().lower() == "borrar":
            db.delete_user(username)
            self.refresh_user_list()
            messagebox.showinfo("Éxito", f"Usuario '{username}' eliminado correctamente.", parent=self)
    ...
    def load_data(self):
        ...
        try:
            ...
            (products, clients, invoices, invoice_counter, iva_rate, exchange_rate, 
             include_pending_in_dashboard, auto_update_rate, rate_source) = db.load_state()
    ...
    def query_dolar_api(self, source_type):
        """Consulta la API de ve.dolarapi.com para obtener la tasa de cambio.
        source_type: 'oficial' o 'paralelo'
        ...
        url = f"https://ve.dolarapi.com/v1/dolares/{source_type}"
        try:
    ...
    def check_and_perform_auto_update(self):
        ...
        if getattr(self, "auto_update_rate", False):
            source = getattr(self, "rate_source", "oficial")
            import threading
            def run():
                rate, err = self.query_dolar_api(source)
                if rate is not None:
    ...
    def apply_auto_updated_rate(self, rate):
        ...
        self.save_data()
        print(f"INFO: Tasa auto-actualizada a {rate:.2f} VES ({self.rate_source})")
